...
(define-condition po-condition (simple-condition)
  ...
  (:report
   (lambda (con stream)
     ...
     (format stream "Post Office condition: ~s~%" (po-condition-identifier con))
     (if* (simple-condition-format-control con)
	  then (apply #'format stream
		      (simple-condition-format-control con)
		      (simple-condition-format-arguments con)))
     (if* (po-condition-server-string con)
	  then (format stream
		       "~&Message from server: ~s"
		       (string-left-trim " " (po-condition-server-string con))))))))
...
(defun po-condition (identifier &key server-string format-control 
		     format-arguments)
  (signal (make-condition 'po-condition
...
(defun make-imap-connection (host &key (port 143) 
			     ...
			     password
			     (timeout 30))
  (let* ((sock (sys:make-fd-stream
		(connect-to-inet-socket host port)
...
(defmethod get-and-parse-from-pop-server ((mb pop-mailbox))
  ...
  (multiple-value-bind (line count)
    ...
    (if* *debug-imap* 
	 then (format t "from server [~D]: " count)
	 (dotimes (i count)
	   (write-char (schar line i)))
	 (terpri))
